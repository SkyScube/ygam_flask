...
def api_conversations():
    ...
    logger.debug("api_conversations: user={}", user.id)
    convs = get_conversations(user)
...
def api_messages(other_user_id):
    user = request.current_user
    logger.debug("api_messages: user={} with={}", user.id, other_user_id)
    history = get_message_history(user, other_user_id)
...
def api_search_users():
    ...
    if len(q) < 2:
        return jsonify([])
    logger.debug("api_search_users: query='{}' user={}", q, request.current_user.id)
